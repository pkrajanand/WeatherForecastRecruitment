...
import org.pactera.weatherforecast.model.Wind;
import org.pactera.weatherforecast.openweathermap.model.WeatherResponseContainerModel;
import org.pactera.weatherforecast.service.adapter.WeatherResponseAdapter;

/**
 * Adapter implementation to transform the open-weather-map weather 
 * response into the weather response model needed by the 
 * weather forecast reporting application 
 * 
 * @author rajanandpk
 *
 */
public class WeatherResponseAdapterImpl implements WeatherResponseAdapter {
    ...
    public WeatherResponseAdapterImpl(WeatherResponseContainerModel model, String unit) {
        if (model == null || unit == null) {
            ...
            this.weatherResponse = new WeatherResponse(model.getName(), new Date(), 
            		model.getWeather(),
